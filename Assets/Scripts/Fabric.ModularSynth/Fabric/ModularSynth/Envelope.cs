	...
	public class Envelope
	{
		...
		private float CalculateValueByType(CurveTypes curveType, float value, float t)
		{
			...
			case CurveTypes.Raised:
				...
				break;
			case CurveTypes.PowerType_1:
				result = ((float)Math.Pow(361.0, t) - 1f) / 360f * value;
				break;
			case CurveTypes.PowerType_2:
				result = ((float)Math.Pow(5.82843017578125, t) - 1f) / 4.82843f * value;
				break;
			case CurveTypes.PowerType_3:
				result = ((float)Math.Pow(2.1512699127197266, t) - 1f) / 1.15127f * value;
				break;
			case CurveTypes.PowerType_4:
				result = ((float)Math.Pow(1.4515000581741333, t) - 1f) / 0.4515f * value;
				break;
			case CurveTypes.PowerType_5:
				result = ((float)Math.Pow(31.9960994720459, t) - 1f) / 30.9961f * value;
				break;
			case CurveTypes.Flat:
			{
				float num2 = t - 0.5f;
				result = 0.5f - 4f * (num2 * num2 * num2);
				break;
			}
			case CurveTypes.Log:
			{
				float num = 1024f;
				float c = (float)Math.Log(1f + num) / (float)Math.Log(2.0);
				float d = -1f;
				result = EnvelopePowerLaw(2f, c, d, t) / num;
				break;
			}
			}
			return result;
		...
		private float EnvelopePowerLaw(float b, float c, float d, float x)
		{
			return (float)Math.Pow(b, c * x) + d;
		}
	}
}
