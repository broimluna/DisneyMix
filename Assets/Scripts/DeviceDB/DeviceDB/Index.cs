	...
	internal class Index<TEntryValue> where TEntryValue : IComparable<TEntryValue>
	{
		...
		public void Insert(uint entryId, TEntryValue value)
		{
			...
			if (entryPosition < num)
			{
				...
				uint num5 = num - 32768;
				uint num6 = num3 / 32768;
				uint num7 = num3 % 32768;
		...
		public void Update(uint entryId, uint oldEntryId, TEntryValue value, TEntryValue oldValue)
		{
			...
			else if (entryIndex > num)
			{
				...
				uint num9 = num8 / 32768;
				uint num10 = num8 % 32768;
		...
		public IEnumerable<KeyValuePair<uint, TEntryValue>> Find(TEntryValue value, Predicate<TEntryValue> matcher)
		{
			...
			if (index < Count)
			{
				...
				ReadEntryAtIndex(index, out readEntryId, out readEntryValue, out readEntryMetadata);
			...
			if (index != 0)
			{
				...
				while (true)
				{
					...
					ReadEntryAtIndex(i, out readEntryId2, out readEntryValue2, out readEntryMetadata2);
			...
			for (uint i2 = index + 1; i2 < Count; i2++)
			{
				uint readEntryId3;
				TEntryValue readEntryValue3;
				uint readEntryMetadata3;
				ReadEntryAtIndex(i2, out readEntryId3, out readEntryValue3, out readEntryMetadata3);
