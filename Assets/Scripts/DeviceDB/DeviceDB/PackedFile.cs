...
namespace DeviceDB
{
	internal class PackedFile
	{
		...
		{
			return (uint)(pagesStream.Length - 1) / 8;
		}
		...
		private Page ReadPage()
		{
			...
			{
				DocumentOffset = pagesReader.ReadUInt32(),
				DocumentLength = pagesReader.ReadUInt16(),
				ReservedDocumentLength = pagesReader.ReadUInt16()
			};
		...
		private void WritePageAtEnd(byte[] document)
		{
			...
			Page page = new Page
			{
				DocumentOffset = (uint)documentsStream.Position,
				DocumentLength = (ushort)document.Length,
				ReservedDocumentLength = (ushort)document.Length
			};
			WritePage(page);
			WriteDocument(page, document);
		}
		...
		{
			if (documentId == 0 || (documentId - 1) % 8 != 0)
			{
