...
namespace DeviceDB
{
	internal class JournalPlayer
	{
		...
		private Header ReadHeader()
		{
			...
			return new Header
			{
				Status = (JournalStatus)reader.ReadByte(),
				NumEntries = reader.ReadInt32(),
				CurEntryIndex = reader.ReadInt32()
			};
		}
		...
		{
			WriteEntry result = new WriteEntry
			{
				FilePosition = reader.ReadInt32(),
				Size = reader.ReadInt32()
			};
			if ((long)result.Size <= 32768L)
			{
				int num = result.Size;
				int num2 = 0;
				do
			...
			{
				result.Bytes = reader.ReadBytes(result.Size);
			}
			return result;
		}
		...
		private CopyEntry ReadCopyEntry()
		{
			...
			{
				SrcPosition = reader.ReadInt32(),
				Size = reader.ReadInt32(),
				DestPosition = reader.ReadInt32()
			};
		}
