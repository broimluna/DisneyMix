...
{
	public class AccessibilityAndroidManager : AccessibilityManager
	{
		...
		{
			return androidPlugin.Call<float>("GetAdjustedFontSize", new object[1] { aFontSize });
		}
		...
		{
			if (Singleton<SettingsManager>.Instance.GetChatVoiceOverSetting())
			{
				androidPlugin.Call("Speak", aTextToSpeak, Singleton<SettingsManager>.Instance.GetChatVoiceOverRateSetting());
			}
		}
