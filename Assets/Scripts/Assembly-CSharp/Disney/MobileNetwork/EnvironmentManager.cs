...
namespace Disney.MobileNetwork
{
	public class EnvironmentManager : MonoBehaviour
	{
		...
		public static ShowAlertDelegate mShowAlertDelegate;
		...
		protected static string mOperatingSystemType;

		protected static Version mOperatingSystemVersion;

		protected static string mOperatingSystemDescription;

		protected static int mMemoryTotalMegabytes;

		protected static string mBundleIdentifier;

		protected static Version mBundleVersion;

		protected static string mBundleVersionCode;

		protected static string mSKU = string.Empty;
		...
		public static string UniversalDeviceIdentifier
		{
			get
			{
				...
				{
					mUDID = EncryptedPlayerPrefs.GetString("mobilenetwork.udid", string.Empty);
					if (string.IsNullOrEmpty(mUDID))
					{
						mUDID = Guid.NewGuid().ToString().ToUpper();
						EncryptedPlayerPrefs.SetString("mobilenetwork.udid", mUDID);
						if (m_instance != null)
						{
							m_instance.Logger.LogInfo(m_instance, "UDID Generated = " + mUDID);
						}
					}
					else if (m_instance != null)
					{
						m_instance.Logger.LogInfo(m_instance, "UDID Loaded = " + mUDID);
					}
				}
				return mUDID;
			}
		...
		protected virtual string _SKU
		{
			get
			{
				...
				{
					mSKU = BuildSettings.Get("mobilenetwork.sku", string.Empty).ToUpper();
				}
				return mSKU;
			}
		}
		...
		protected virtual string _SKUPluginManifestJson
		{
			get
			{
				return BuildSettings.Get("mobilenetwork.pluginManifestJson", string.Empty);
			}
		}
		...
		private void _PopulateOperatingSystemInfo()
		{
			...
			foreach (string text3 in array)
			{
				if (mOperatingSystemDescription.StartsWith(text3))
				{
					...
					if (text.Contains("Android OS N"))
					{
						text = mOperatingSystemType + " 7.0.0";
					}
					else if (text.IndexOf(".") < 0)
					{
						text = mOperatingSystemType + " 1.0.0";
					}
		...
		{
			ShowAlert(showAlertDelegate, string.Empty, message, viewButtonText, cancelButtonText);
		}
