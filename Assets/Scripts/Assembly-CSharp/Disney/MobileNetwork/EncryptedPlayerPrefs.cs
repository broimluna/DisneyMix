namespace Disney.MobileNetwork
{
	public class EncryptedPlayerPrefs
	{
		...
		public static float GetFloat(string key, float defaultValue)
		{
			...
			{
				float num = s_priorImpl.GetFloat(key, defaultValue);
				s_priorImpl.DeleteKey(key);
				s_impl.SetFloat(key, num);
				return num;
			}
		...
		public static int GetInt(string key, int defaultValue)
		{
			...
			if (s_priorImpl != null && s_priorImpl.HasKey(key))
			{
				int num = s_priorImpl.GetInt(key, defaultValue);
				s_priorImpl.DeleteKey(key);
				s_impl.SetInt(key, num);
				return num;
			}
		...
		public static string GetString(string key, string defaultValue)
		{
			...
			if (s_priorImpl != null && s_priorImpl.HasKey(key))
			{
				string text = s_priorImpl.GetString(key, defaultValue);
				s_priorImpl.DeleteKey(key);
				s_impl.SetString(key, text);
				return text;
			}
