...
namespace Disney.Mix.SDK.Internal
{
	public class AgeBandBuilder : IAgeBandBuilder
	{
		...
		{
			IRegistrationPermissions permissions = CreateRegistrationPermissions((!registration) ? configurationAgeBand.UPDATE : configurationAgeBand.CREATE);
		...
		private static IEnumerable<string> GetLegalDocumentsTypeOrder(IDictionary<string, LegalGroup> groups, string ageBandKey)
		{
			...
			if (legalGroup != null)
			{
				IEnumerable<string> documentTypeOrder = legalGroup.documentTypeOrder;
				result = documentTypeOrder;
			}
			else
			{
		...
		{
			LegalGroup legalGroup = DictionaryUtils.TryGetValue(groups, ageBandKey);
			IEnumerable<ILegalDocument> enumerable2;
			if (UseCreate(registration, legalGroup))
			{
				IEnumerable<ILegalDocument> enumerable = CreateDocuments(legalGroup.CREATE, legalTexts);
				enumerable2 = enumerable;
			}
			else
			{
				enumerable2 = CreateDocuments(legalGroup.documents, legalTexts);
			}
			IEnumerable<ILegalDocument> source = enumerable2;
			List<ILegalDocument> list = new List<ILegalDocument>(source.Count());
			string type;
			foreach (string item in legalGroup.documentTypeOrder)
			{
				type = item;
				IEnumerable<ILegalDocument> collection = source.Where((ILegalDocument doc) => doc.Type == type);
		...
		private static IEnumerable<string> GetLegalTextIds(SiteConfigurationData siteConfig, string ageBandKey, bool registration)
		{
			...
			if (UseCreate(registration, legalGroup))
			{
				IEnumerable<string> enumerable = legalGroup.CREATE.Select((LegalProxy proxy) => CreateTextId(proxy));
				result = enumerable;
			}
			else
			{
				result = legalGroup.documents.Keys.Distinct();
			}
			return result;
		}
		...
		private static Dictionary<string, ConfigurationMarketingItem> GetConfiguration(ConfigurationMarketingAgeBand ageBand, bool registration)
		{
			...
			return (!registration) ? ageBand.PARTIAL : ageBand.CREATE;
		}
		...
		private static IEnumerable<IMarketingItem> GetMarketingItems(Dictionary<string, ConfigurationMarketingItem> context, IEnumerable<RegistrationText> legalTexts)
		{
			if (context != null)
			{
				return context.Select(delegate(KeyValuePair<string, ConfigurationMarketingItem> pair)
				{
					...
					return new MarketingItem(id, registrationText.Text, itemType, isChecked);
				}).ToArray();
			}
