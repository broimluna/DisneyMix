...
{
	public class NotificationPoller : IDisposable, INotificationPoller
	{
		...
		public int MaximumMissingNotificationTime { private get; set; }
		...
		private int[] CurIntervals
		{
			get
			{
				return (!UsePollIntervals) ? pokeIntervals : pollIntervals;
			}
		}
		...
		public NotificationPoller(AbstractLogger logger, IMixWebCallFactory webCallFactory, INotificationQueue queue, IStopwatch pollCountdownStopwatch, IGetStateResponseParser getStateResponseParser, IEpochTime epochTime, IRandom random, IDatabase database, string localUserSwid)
		{
			...
			PollIntervals = new int[1] { 2147483647 };
			PokeIntervals = new int[1] { 2147483647 };
		...
		private void HandleGetNotificationsResponse(GetNotificationsResponse response)
		{
			...
			// Handle the case where NotificationSequenceCounter is null (no new notifications/updates)
			long value = response.NotificationSequenceCounter.HasValue ? response.NotificationSequenceCounter.Value : queue.LatestSequenceNumber;
		...
		{
			// Instead of enforcing NotificationSequenceCounter to have a value,
			// just ensure a valid Status since an empty response is technically OK.
			return response.Status == null || response.Status == "OK" || response.NotificationSequenceCounter.HasValue;
		}
		...
		{
			notifications.RemoveAll(delegate(BaseNotification n)
			{
				long? sequenceNumber = n.SequenceNumber;
				return (sequenceNumber.HasValue && sequenceNumber.Value <= queue.LatestSequenceNumber) || queue.IsQueued(n.SequenceNumber.Value);
			});
		}

		private static void SortNotificationsBySequenceNumber(List<BaseNotification> notifications)
		{
			notifications.Sort(delegate(BaseNotification a, BaseNotification b)
			{
