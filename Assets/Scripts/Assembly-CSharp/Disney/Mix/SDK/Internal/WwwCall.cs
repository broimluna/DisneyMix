...
{
	public class WwwCall : IDisposable, IWwwCall
	{
		...
		public long TimeToStartUpload { get; private set; }

		public long TimeToFinishUpload { get; private set; }
		...
		public long TimeToStartDownload { get; private set; }

		public long TimeToFinishDownload { get; private set; }
		...
		private IEnumerator RunWww()
		{
			...
			while (!www.IsDone)
			{
				...
				ResponseBody = wwwDefaultResponseBody;
				ResponseHeaders = wwwDefaultResponseHeaders;
				TimeoutReason = ((elapsedTime < latencyWwwCallTimeout) ? "Whole process took too long" : "Response didn't start in time");
				this.OnDone(this, new WwwDoneEventArgs());
				yield break;
			}
			stopwatch.Stop();
