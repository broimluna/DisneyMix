...
{
	public class GroupChatThread : AbstractChatThread, IInternalChatThread, IInternalGroupChatThread, IChatThread, IGroupChatThread
	{
		...
		public void AddMembers(IEnumerable<IInternalFriend> friends, Action<IAddChatThreadMemberResult> callback)
		{
			ChatThreadMemberAdder.AddMembers(logger, notificationQueue, mixWebCallFactory, friends.Select((IInternalFriend f) => f.Swid), chatThreadId, delegate
			{
				callback(new AddChatThreadMemberResult(true));
			}, callback);
		}

		public void RemoveRemoteMember(IInternalRemoteChatMember member, Action<IRemoveChatThreadMemberResult> callback)
		{
			ChatThreadMemberRemover.RemoveMember(logger, notificationQueue, mixWebCallFactory, member.Swid, chatThreadId, delegate
			{
				callback(new RemoveChatThreadMemberResult(true));
			}, delegate
			{
		...
		public void RemoveLocalUser(IInternalLocalUser member, Action<IRemoveChatThreadMemberResult> callback)
		{
			ChatThreadMemberRemover.RemoveMember(logger, notificationQueue, mixWebCallFactory, member.Swid, chatThreadId, delegate
			{
				callback(new RemoveChatThreadMemberResult(true));
				DispatchMemberRemovedEvent(new ChatThreadMemberRemovedEventArgs(member.Id));
			}, delegate
			{
				callback(new RemoveChatThreadMemberResult(false));
			});
		 ...
		 void IGroupChatThread.RemoveNickname(Action<IRemoveChatThreadNicknameResult> callback)
		{
		 ...
		 IGagMessage IGroupChatThread.SendGagMessage(string contentId, IRemoteChatMember targetUser, Action<ISendGagMessageResult> callback)
		{
