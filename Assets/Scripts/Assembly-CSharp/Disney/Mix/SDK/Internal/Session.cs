...
{
	public class Session : IDisposable, IInternalSession, ISession
	{
		...
		public string GuestControllerAccessToken { get; private set; }
		...
		private IEnumerator Update()
		{
			while (!IsDisposed)
			{
				if (!sessionStatus.IsPaused)
				{
					...
					{
						Exception ex2 = ex;
						logger.Critical("Exception during Update(): " + ex2);
					}
				}
				yield return null;
			}
		...
		public void Resume(Action<IResumeSessionResult> callback)
		{
			...
			try
			{
				SessionResumer.Resume(logger, getStateResponseParser, epochTime, clientVersion, notificationQueue, mixWebCallFactory, localUser, database, userDatabase, chatMessageParser, notificationPoller, chatMessageHandler, delegate(IResumeSessionResult result)
				{
					if (result.Success)
					{
			...
			{
				logger.Critical("Error while resuming: " + ex);
		...
		{
			Dispose();
			this.OnTerminated(this, new LocalStorageCorruptedEventArgs());
		}
