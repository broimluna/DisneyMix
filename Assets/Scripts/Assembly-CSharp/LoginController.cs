...
public class LoginController : BaseController
{
	...
	private NativeTextView usernameField;

	private NativeTextView passwordField;
	...
	private void Start()
	{
		...
		usernameField = UsernameGameObject.GetComponent<NativeTextView>();
		passwordField = PasswordGameObject.GetComponent<NativeTextView>();
	...
	private void Update()
	{
		QuestionMarkButton.SetActive(string.IsNullOrEmpty(usernameField.Value));
		if (!loginSubmitted)
		{
			if (string.IsNullOrEmpty(usernameField.Value) || string.IsNullOrEmpty(passwordField.Value) || string.Empty.Equals(usernameField.Value.Trim()) || string.Empty.Equals(passwordField.Value.Trim()))
			{
	...
	public unsafe override void OnUITransitionEnd()
	{
	...
	public override void OnAndroidBackButtonClicked()
	{
		if (!Singleton<PanelManager>.Instance.OnAndroidBackButton())
		{
			...
			{
				FGAnimator.Play("AccountLogin_BacktoSplash");
	...
	public void onLogin()
	{
		...
		if (ValidateInput())
		{
			...
			MonoSingleton<LoginManager>.Instance.Login(usernameField.Value, passwordField.Value, handleLoginResponse);
	...
	{
		if (usernameField.Selected || passwordField.Selected)
		{
			MonoSingleton<NativeKeyboardManager>.Instance.Keyboard.Hide();
			return;
		}
	...
	public void handleLoginResponse(ILoginResult loginResult)
	{
		...
		if (loginResult is ILoginFailedMultipleAccountsResult)
		{
			...
			navigationRequest2.AddData("MASE_username", usernameField.Value);
	...
	{
		return !string.IsNullOrEmpty(usernameField.Value) && !string.IsNullOrEmpty(passwordField.Value) && !string.Empty.Equals(usernameField.Value.Trim()) && !string.Empty.Equals(passwordField.Value.Trim());
	}
