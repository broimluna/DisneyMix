...
public class TextWithEvents : Text
{
	...
	{
		strToParse = strToParse.Replace("<a", "<color=#0000ffff><a");
		strToParse = strToParse.Replace("</a>", "</a></color>");
	...
	protected override void OnPopulateMesh(VertexHelper vh)
	{
		...
		foreach (KeyValuePair<string, List<int[]>> item in charsIdForClass)
		{
			...
			{
				if (textGenerator.lineCount > 1 && textGenerator.verts[item2[0] * 4 + 2].position.y > textGenerator.verts[(item2[1] * 4 + 3 <= textGenerator.vertexCount - 1) ? (item2[1] * 4) : (textGenerator.vertexCount - 4)].position.y)
				{
					int num = 1;
