	...
	public class FindFriend : MonoBehaviour, IFriendListItem
	{
		...
		private void Update()
		{
			if (SearchButton != null)
			{
				...
				SearchButton.gameObject.SetActive(Input.Selected);
		...
		public void Init(IFriendListItem aListItemListener, Action<IOutgoingFriendInvitation> aFriendInvitedCallback)
		{
			...
			Input.KeyboardValueChanged += OnSearchValueChanged;
			Input.KeyboardHeightChanged += OnKeyboardHeightChanged;
		}
		...
		{
			SearchForFriendByDisplayName(Input.Value);
		...
		private void OnKeyboardHeightChanged(NativeTextView aField, int aHeight)
		{
			if (!(aField != Input))
			{
				ResizeScrollView(aHeight);
			}
		}

		private void OnSearchValueChanged(NativeTextView aField, string aValue)
		{
			SetValue(aValue);
		}
