...
namespace Mix.Ui
{
	...
	{
		public NativeTextView DisplayNameInputField;
		...
		public void Init(IDisplayNamePanelCallback aCaller, IRegistrationProfile aUserInfo)
		{
			...
			submitted = false;
			DisplayNameInputField.KeyboardFocusChanged -= InputFocusChanged;
			DisplayNameInputField.KeyboardFocusChanged += InputFocusChanged;
		}
		...
		private void Update()
		{
			if (!submitted)
			{
				SubmitDisplayNameButton.interactable = !DataChecker.IsNullEmptyOrJustWhiteSpace(DisplayNameInputField.Value);
			}
		}
		...
		public void OnSubmitDisplayName()
		{
			...
			submitted = true;
			if (ClientValidateValues() && IsAlphaNumeric(DisplayNameInputField.Value))
			{
				MixSession.User.UpdateDisplayName(DisplayNameInputField.Value, delegate(IUpdateDisplayNameResult result)
				{
					if (!this.IsNullOrDisposed() && !(base.gameObject == null))
					{
						if (result.Success)
						{
							...
							ShowDisplayNameAccepted(DisplayNameInputField.Value, delegate
							{
							});
						}
						else
		...
		{
			errorDisplayName = DataChecker.IsNullEmptyOrJustWhiteSpace(DisplayNameInputField.Value);
			if (errorDisplayName)
			{
				ErrorMessageGameObject.GetComponent<Text>().text = Singleton<Localizer>.Instance.getString("customtokens.register.error_display_name_invalid");
			}
			...
			{
				errorDisplayName = DisplayNameInputField.Value.Length < 4 || DisplayNameInputField.Value.Length > 15;
				if (errorDisplayName)
				{
