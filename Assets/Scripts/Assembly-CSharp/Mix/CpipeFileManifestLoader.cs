...
namespace Mix
{
	public class CpipeFileManifestLoader : ITextAssetObject
	{
		...
		public void Load(CpipeCallback onComplete, CpipeErrorCallback onError, bool aIsFirstRun, CachePolicy aCachePolicy, string oldVersion)
		{
			...
			ContentVersion = MonoSingleton<AssetManager>.Instance.GetBuildNumber();
			string aEnvironment = MonoSingleton<AssetManager>.Instance.GetEnvironment().ToString().ToLower();
			Singleton<CpipeManifestApi>.Instance.GetCpipeManifest(aEnvironment, ContentVersion, this.oldVersion, this, aCachePolicy);
		}
