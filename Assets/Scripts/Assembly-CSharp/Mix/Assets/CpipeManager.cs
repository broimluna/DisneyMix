...
namespace Mix.Assets
{
	public class CpipeManager
	{
		...
		public void Update()
		{
			if (IsUseAssetAuth() && !IsRefreshingToken)
			{
				AuthSecondsPast += Time.deltaTime;
				if ((float)AuthTimeoutStartSeconds + AuthSecondsPast >= (float)ExpireSeconds)
				{
					AuthSecondsPast = 0f;
					refreshToken();
				}
			}
			TimeSinceLastCpipeRefresh += Time.deltaTime;
			if (TimeSinceLastCpipeRefresh > (float)CpipeRefreshTime)
			{
				TimeSinceLastCpipeRefresh = 0f;
				LoadCpipe(CachePolicy.DefaultCacheControlProtocol);
			}
		}
