...
namespace ZXing.Common.Detector
{
	public sealed class MonochromeRectangleDetector
	{
		...
		private int[] blackWhiteRange(int fixedDimension, int maxWhiteRun, int minDim, int maxDim, bool horizontal)
		{
			...
			while (num2 >= minDim)
			{
				if ((!horizontal) ? image[fixedDimension, num2] : image[num2, fixedDimension])
				{
					num2--;
					continue;
				}
				...
				while (num2 >= minDim && !((!horizontal) ? image[fixedDimension, num2] : image[num2, fixedDimension]));
				int num4 = num3 - num2;
			...
			while (num5 < maxDim)
			{
				if ((!horizontal) ? image[fixedDimension, num5] : image[num5, fixedDimension])
				{
					num5++;
					continue;
				}
				...
				while (num5 < maxDim && !((!horizontal) ? image[fixedDimension, num5] : image[num5, fixedDimension]));
				int num7 = num5 - num6;
