...
namespace ZXing.QrCode.Internal
{
	public class Detector
	{
		...
		{
			resultPointCallback = ((hints != null && hints.ContainsKey(DecodeHintType.NEED_RESULT_POINT_CALLBACK)) ? ((ResultPointCallback)hints[DecodeHintType.NEED_RESULT_POINT_CALLBACK]) : null);
			FinderPatternFinder finderPatternFinder = new FinderPatternFinder(image, resultPointCallback);
		...
		protected internal virtual DetectorResult processFinderPatternInfo(FinderPatternInfo info)
		{
			...
			int dimension;
			if (!computeDimension(topLeft, topRight, bottomLeft, num, out dimension))
			{
				return null;
			}
		...
		private static BitMatrix sampleGrid(BitMatrix image, PerspectiveTransform transform, int dimension)
		{
			...
			return instance.sampleGrid(image, dimension, dimension, transform);
		}
		...
		private float sizeOfBlackWhiteBlackRun(int fromX, int fromY, int toX, int toY)
		{
			...
			for (; i != num8; i += num5)
			{
				...
				if (num7 == 1 == image[x, y])
				{
					if (num7 == 2)
					{
						return MathUtils.distance(i, num9, fromX, fromY);
					}
					num7++;
				}
				num4 += num3;
